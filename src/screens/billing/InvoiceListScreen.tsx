...
export function InvoiceListScreen({ navigation }: InvoiceListScreenProps) {
  ...
  const loadInvoices = async () => {
    try {
      ...
      const mapped: InvoiceListItem[] = rows.map((row) => {
        ...
        const legalTip = resolveInvoiceLegalTip(parsedData, Number(parsedData?.subtotalCents ?? 0));
        return {
  ...
  const resolveInvoiceShippingCents = (params: {
    ...
    const inferredShipping = Math.round(
      Number(params.totalCents || 0) - calculatedWithoutShipping - Number(params.legalTipCents || 0)
    );
    return inferredShipping > 0 ? inferredShipping : 0;
  };

  const resolveInvoiceLegalTip = (parsedData: any, fallbackBaseCents: number = 0) => {
    const summary = resolveLegalTipSummary(parsedData, fallbackBaseCents);
    return {
  ...
  const buildThermalInvoiceHtml = (params: {
    ...
    const taxRows = showItbisBreakdown
      ? `<div class="row"><span>Subtotal</span><span>${formatCurrency(totals.subtotalCents)}</span></div><div class="row"><span>ITBIS (${salePricesIncludeItbis ? 'incluido' : 'no incluido'})</span><span>${formatCurrency(totals.itbisCents)}</span></div>${discountRow}${shippingRow}${legalTipRow}`
  ...
  const handlePrintInvoice = async (invoice: InvoiceListItem) => {
    try {
      ...
      const legalTipFallbackBaseCents = calcDocumentTotalsByTaxMode({
        items: items.map((item: any) => ({
          quantity: Number(item.quantity || item.qty || 0),
          priceCents: Number(item.priceCents || item.unitPriceCents || 0),
          itbisRateBp: Number(item.itbisRateBp || 1800),
      ...
      const printResult = await printSaleTicketDirect({
        ...
        discountTotalCents,
        legalTipPercentBp: legalTip.legalTipPercentBp,
        legalTipBaseCents: legalTipFallbackBaseCents,
        legalTipCents: legalTip.legalTipCents,
        shippingCents,
        totalCents,
        salePricesIncludeItbis,
  ...
  const renderInvoice = ({ item }: { item: InvoiceListItem }) => (
    <View style={styles.invoiceCard}>
      ...
      {Number(item.legalTipCents || 0) > 0 ? (
        <Text style={styles.meta}>
          Propina legal ({(Number(item.legalTipPercentBp || LEGAL_TIP_PERCENT_BP) / 100).toFixed(2).replace(/\.?0+$/, '')}%): {formatCurrency(Number(item.legalTipCents || 0))}
        </Text>
      ) : null}
